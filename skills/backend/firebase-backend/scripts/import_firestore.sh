...
if [ "$TO_EMULATOR" = true ]; then
    ...
    if [ -z "$PROJECT" ]; then
        echo "ERROR: No active project. Run: firebase use <project-id>"
        exit 1
    fi
    ...
    echo "WARNING: This imports data to PRODUCTION Firestore"
    ...
    if [ "$CONFIRM_PROJECT" != "$PROJECT" ]; then
        echo "ERROR: Project ID doesn't match. Import cancelled."
        exit 1
    fi

    # Validate GCS path for production
    if [[ "$INPUT_PATH" != gs://* ]]; then
        echo "ERROR: Production import requires a GCS bucket path (gs://bucket-name/path)"
        exit 1
    fi
