...
**也會被自動觸發提示**：`run_master_chain.sh daily`（每日 21:02 systemd timer）在 `sync-filers`
後跑 `reconcile_filers.sh --check`。偵測到退場候選時，當日 `logs/master_chain_*.log` 會出現
`⚠️⚠️ 偵測到 N 家退場候選 … 請執行 /mops-filer-list-reconcile`，且 `logs/reconcile_candidates.txt`
為非空。看到該提示（或使用者貼上該 log）就進入下面流程。**排程只偵測、不刪**——刪除一律走
本 skill 的人工 gate（systemd 無法跑互動式 skill，刻意如此以保安全）。
